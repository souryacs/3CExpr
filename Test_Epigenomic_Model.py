...
def main():
    ...
    if prediction == True:
        ...
        for cagefileidx in range(len(CAGETrackList)):
            ...
            SummaryDF = pd.DataFrame(data={'Validation_chr': [options.valid_chr], 'Test_chr': [options.test_chr], 'NLL_GAT_MinExpr_0_MinContact_0': [valid_loss_gat[0].astype(np.int64)], 'NLL_GAT_MinExpr_5_MinContact_0': [valid_loss_gat[1].astype(np.int64)], 'NLL_GAT_MinExpr_5_MinContact_1': [valid_loss_gat[2].astype(np.int64)], 'NLL_GAT_MinExpr_5_MinContact_5': [valid_loss_gat[3].astype(np.int64)], 'NLL_GAT_Mean': [np.mean(valid_loss_gat, axis=0)], 'NLL_GAT_std': [np.std(valid_loss_gat, axis=0)], 'rho_MinExpr_0_MinContact_0': [valid_rho_gat[0].astype(np.int64)], 'rho_MinExpr_5_MinContact_0': [valid_rho_gat[1].astype(np.int64)], 'rho_MinExpr_5_MinContact_1': [valid_rho_gat[2].astype(np.int64)], 'rho_MinExpr_5_MinContact_5': [valid_rho_gat[3].astype(np.int64)], 'rho_Mean': [np.mean(valid_rho_gat, axis=0)], 'rho_std': [np.std(valid_rho_gat, axis=0)], 'sp_MinExpr_0_MinContact_0': [valid_sp_gat[0].astype(np.int64)], 'sp_MinExpr_5_MinContact_0': [valid_sp_gat[1].astype(np.int64)], 'sp_MinExpr_5_MinContact_1': [valid_sp_gat[2].astype(np.int64)], 'sp_MinExpr_5_MinContact_5': [valid_sp_gat[3].astype(np.int64)], 'sp_Mean': [np.mean(valid_sp_gat, axis=0)], 'sp_std': [np.std(valid_sp_gat, axis=0)]})
